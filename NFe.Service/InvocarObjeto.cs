...
namespace NFe.Service
{
    ...
    public class InvocarObjeto
    {
        ...
        public XmlDocument InvocarNFSeAPI(XmlDocument xml,
                            ...
                            SecurityProtocolType securityProtocolType)
        {
            ...
            // Validar o Arquivo XML
            ValidarXML validar = new ValidarXML(xml, Empresas.Configuracoes[emp].UnidadeFederativaCodigo, false);
            validar.IsAPI = true;

            string cResultadoValidacao = validar.ValidarArqXML(ref xml);
            if (cResultadoValidacao != "")
            {
                switch (padraoNFSe)
                {
            ...
            docRet.Load(Functions.StringXmlToStreamUTF8(strRetorno));

            return docRet;
            // Registra o retorno de acordo com o status obtido
            //if (finalArqEnvio != string.Empty && finalArqRetorno != string.Empty)
            //{
            //   return (XmlDocument)typeServicoNFe.InvokeMember("XmlRetorno", System.Reflection.BindingFlags.InvokeMethod, null, servicoNFe, 
            //        new Object[] { finalArqEnvio + ".xml", finalArqRetorno + ".xml" });
            //}
        }
