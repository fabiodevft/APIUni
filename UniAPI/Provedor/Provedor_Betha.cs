...
{
    public class Provedor_Betha : AbstractProvedor, IProvedor
    {
        internal Provedor_Betha()
        {
            this.Nome = EnumProvedor.Betha;
        }
        ...
        private XmlElement CriaHeaderXml(string prefixo, string strNomeMetodo, ref XmlDocument doc)
        {
            ...
            var gerarNotaNode = doc.CreateElement(prefixo, strNomeMetodo, "http://www.betha.com.br/e-nota-contribuinte-ws");
        ...
        {
            if (nota.Provedor.Nome != EnumProvedor.Betha)
            {
            ...
            var bRps = false;
            string strOutrasInformacoes = string.Empty;
                                ...
                                case EnumResposta.EnviarLoteRpsResposta:
                                    {
                                        switch (x.Name.ToString().ToLower())
                                        {
                                            ...
                                            case "outrasinformacoes":
                                                strOutrasInformacoes = x.ReadString();
                                                break;
            ...
            if (strOutrasInformacoes.Contains(@"http://e-gov.betha.com.br/e-nota"))
            {
                linkImpressaoAux = strOutrasInformacoes.Substring(strOutrasInformacoes.LastIndexOf(@"http://e-gov.betha.com.br/e-nota"));
            }
        ...
        public override XmlDocument GeraXmlNota(NFSeNota nota)
        {
            ...
            if (nota.Documento.TDFe.TServico.FExigibilidadeISS.ToString().Equals("1")
                || nota.Documento.TDFe.TServico.FExigibilidadeISS.ToString().Equals("6")
                || nota.Documento.TDFe.TServico.FExigibilidadeISS.ToString().Equals("7"))
            {
        ...
        {
            var doc = new XmlDocument();
            var gerarNotaNode = CriaHeaderXml("e", "ConsultarLoteRpsEnvio", ref doc);
