...
namespace NFe.Components.Betha.NewVersion
{
    public abstract class BethaBase : EmiteNFSeBase, Ambiente.IAmbiente
    {
        ...
        public BethaBase(TipoAmbiente tpAmb, int codMun, string usuario, string senhaWs, string proxyuser, string proxypass, string proxyserver)
            : base(tpAmb)
        {
            CodigoMun = codMun;
            Usuario = usuario;
        ...
        public XmlDocument EmiteNFSincrono(XmlDocument file)
        {
            Ambiente.IAmbiente service = BethaService as Ambiente.IAmbiente;

            return service.EmiteNFSincrono(file);
        }

        public XmlDocument EmiteNF(XmlDocument xml)
        {
            Ambiente.IAmbiente service = BethaService as Ambiente.IAmbiente;
