...
namespace NFe.Validate
{
    ...
    public class ValidarXML
    {
        ...
        public ValidarXML(XmlDocument conteudoXML, int UFCod, bool soValidar, Propriedade.TipoEnvio tipoMetodo)
        {
            TipoArqXml = new TipoArquivoXML("", conteudoXML, UFCod, soValidar);
            this.tpEnvio = tipoMetodo;
        }
        ...
        public Propriedade.TipoEnvio tpEnvio { get; set; }
        ...
        public void EncryptAssinatura(ref XmlDocument arquivoXML)
        {
            if (TipoArqXml.cArquivoSchema.Contains("DSF\\SJCSP"))
                return;
                ...
                TipoArqXml.cArquivoSchema.Contains("DSF"))
            {
                //if (arquivoXML.EndsWith(Propriedade.Extensao(Propriedade.TipoEnvio.EnvLoteRps).EnvioXML, StringComparison.InvariantCultureIgnoreCase) ||
                //    arquivoXML.EndsWith(Propriedade.Extensao(Propriedade.TipoEnvio.PedCanNFSe).EnvioXML, StringComparison.InvariantCultureIgnoreCase))
                if(tpEnvio == Propriedade.TipoEnvio.EnvLoteRps || tpEnvio == Propriedade.TipoEnvio.PedCanNFSe)
                {
                    bool found = false;
                    bool bSave = false;
                    string sh1 = "";
                    ...
                    //if (arquivoXML.EndsWith(Propriedade.Extensao(Propriedade.TipoEnvio.EnvLoteRps).EnvioXML, StringComparison.InvariantCultureIgnoreCase))
                    if(tpEnvio == Propriedade.TipoEnvio.EnvLoteRps)
                    {
                        const string Assinatura = "Assinatura";
                    ...
                    //        !TipoArqXml.cArquivoSchema.Contains("DSF"))
                    if(tpEnvio == Propriedade.TipoEnvio.PedCanNFSe && !TipoArqXml.cArquivoSchema.Contains("DSF"))
                    {
                        const string AssinaturaCancelamento = "AssinaturaCancelamento";
        ...
        private void Validar(ref XmlDocument conteudoXML)
        {
            Retorno = 0;
            RetornoString = "";
            ...
            if (File.Exists(TipoArqXml.cArquivoSchema))
            {
                ...
                try
                {
                    ...
                    EncryptAssinatura(ref conteudoXML);    //danasa: 12/2013
        ...
        public string ValidarArqXML(string arquivo)
        {
            ...
            {
                if (!IsAPI) //API NÃO UTILIZA ARQUIVO FISICO
                {
                    Validar(arquivo);
                }
                else
        ...
        public string ValidarArqXML(ref XmlDocument arquivo)
        {
            string cRetorna = "";
