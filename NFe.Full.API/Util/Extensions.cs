...
namespace NFe.Full.API.Util
{
    public static class Extensions
    {

        public static XmlNode CriarNoNotNull(XmlDocument doc, XmlNode node, string nome, string valor)
        {
            if (valor != null && valor != "")
                return CriarNo(doc, node, nome, valor, "", "");
            else
                return null;
        }

        public static XmlNode CriarNoNotNull(XmlDocument doc, XmlNode node, string nome, string valor, string[] prefixo)
        {
            if (valor != null && valor != "")
                return CriarNo(doc, node, nome, valor, "", "", prefixo);
            else
                return null;
        }

        public static XmlNode CriarNoNotNull(XmlDocument doc, XmlNode node, string nome, string valor, string atributo, string atributoValor)
        {
            if (valor != "")
                return CriarNo(doc, node, nome, valor, atributo, atributoValor);
            else
                return null;
        }

        public static XmlNode CriarNoNotNull(XmlDocument doc, XmlNode node, string nome, string valor, string atributo, string atributoValor, string[] prefixo)
        {
            if (valor != "")
                return CriarNo(doc, node, nome, valor, atributo, atributoValor, prefixo);
            else
                return null;
        }

        public static XmlNode CriarNo(XmlDocument doc, XmlNode node, string nome)
        {
            return CriarNo(doc, node, nome, "", "", "");
        }

        public static XmlNode CriarNo(XmlDocument doc, XmlNode node, string nome, string valor)
        {
            return CriarNo(doc, node, nome, valor, "", "");
        }

        public static XmlNode CriarNo(XmlDocument doc, XmlNode node, string nome, string valor, string[] prefixo)
        {
            return CriarNo(doc, node, nome, valor, "", "", prefixo);
        }

        public static XmlNode CriarNo(XmlDocument doc, XmlNode node, string nome, string valor, string atributo, string atributoValor)
        {
            XmlNode nodeItem = doc.CreateElement(nome);
            ...
            {
                var loteId = doc.CreateAttribute(atributo);
                loteId.Value = atributoValor;
                nodeItem.Attributes.Append(loteId);
            }

            return nodeItem;
        }

        public static XmlNode CriarNo(XmlDocument doc, XmlNode node, string nome, string valor, string atributo, string atributoValor, string[] prefixo)
        {
            if (prefixo.Length == 2)
            {
                if (prefixo[0] != "" && prefixo[1] != "")
                {
                    XmlNode nodeItem = doc.CreateElement(prefixo[0], nome, prefixo[1]);
                    ...
                    if (!string.IsNullOrEmpty(atributo))
                    {
                        var loteId = doc.CreateAttribute(prefixo[0], atributo, prefixo[1]);
                        loteId.Value = atributoValor;
                        nodeItem.Attributes.Append(loteId);
                    }
        ...
        public static bool PossuiCasasDecimais(decimal valor)
        {
            var resultado = valor - Math.Truncate(valor);
            return (resultado != 0);
        }
        ...
        public static class Strings
        {
            ...
            public static string SHA1(string valor)
            {
                using (SHA1 sha = new SHA1CryptoServiceProvider())
                {
                    ...
                    var combined = encoder.GetBytes(valor);
                    return BitConverter.ToString(sha.ComputeHash(combined)).Replace("-", "").ToLower();
                }
            }

            public static string FoneComDDD(string ddd, string fone)
            {
                var dddFone = ddd ?? "";
                if (dddFone != "")
                    dddFone = string.Concat(dddFone, " ");
                dddFone = string.Concat(dddFone, fone ?? "");
        ...
        public enum TTiposDadoEDI
        {
            ...
            /// <summary>
            /// Representa um campo de data no formato aaaamm. A propriedade ValorNatural é do tipo DateTime, com o dia igual a 01
            /// </summary>
            ediDataAAAAMM___________,
            /// <summary>
