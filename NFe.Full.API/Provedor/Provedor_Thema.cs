...
{
    internal class Provedor_Thema : AbstractProvedor, IProvedor
    {
        internal Provedor_Thema()
        {
            this.Nome = EnumProvedor.Thema;
        }
        ...
        {
            if (nota.Provedor.Nome != EnumProvedor.Thema)
            {
        ...
        public override XmlDocument GeraXmlNota(NFSeNota nota)
        {
            ...
            Extensions.CriarNoNotNull(doc, ServicoNode, "ItemListaServico",Generico.RetornarApenasNumeros(nota.Documento.TDFe.TServico.FItemListaServico));
        ...
        public override XmlDocument GerarXmlCancelaNota(NFSeNota nota, string numeroNFSe)
        {
            ...
            Extensions.CriarNo(doc, InfPedidoCancelamentoNode, "CodigoCancelamento", "E506"); // tsCodigoCancelamentoNfse
        ...
        {
            /* tsNaturezaOperacao - Código de natureza da operação *** SEM PONTOS ****
                    5.2 - Imposto devido em Cachoeirinha, sem obrigação
                          de retenção na fonte,
                    5.3 - Imposto devido em Cachoeirinha com obrigação de
                    retenção na fonte,
                    5.8 - Não Tributável,
                    5.9 - Imposto recolhido pelo regime único de
                    arrecadação,
                    6.2 - Imposto devido em Cachoeirinha, sem obrigação
                    de retenção na fonte,
                    6.3 - Imposto devido fora de Cachoeirinha, com
                    obrigação de retenção na fonte,
                    6.4 - Imposto devido fora de Cachoeirinha, sem
                    obrigação de retenção na fonte,
                    6.8 - Não tributável, 6.9 - Imposto recolhido pelo regime
                    único de arrecadação,
                    7.8 - Não tributável,
                    7.9 - Imposto recolhido pelo regime único de
                    arrecadação,
                    8.1 - Imposto recolhido por guia sem escrituração*/
            ...
            if( (EnumNFSeSituacaoTributaria) nota.Documento.TDFe.TServico.FValores.FIssRetido ==  EnumNFSeSituacaoTributaria.stRetencao ){
                ...
                { return "63"; }else { return "53"; }
                    
            }else{
                if (nota.Documento.TDFe.TServico.FMunicipioIncidencia != nota.Documento.TDFe.TPrestador.TEndereco.FCodigoMunicipio)
                { return "62"; }else { return "52"; }
            }           
        }
    }
}
